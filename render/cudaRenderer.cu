...
__global__ void kernelRenderCircles() {
    ...
    for(size_t indexCicle=0; indexCicle < numberOfCircles; indexCicle += numberCirclesPerIteration){
        size_t maxNumberCircles = numberCirclesPerIteration < numberOfCircles - indexCicle ? numberCirclesPerIteration : numberOfCircles - indexCicle;
        
        int index = threadIdx.x + blockIdx.x * threadIdx.y;
        for(size_t i=index; i<maxNumberCircles; i+=numberThreadsOfBlock){
            size_t circleIndex = indexCicle + i;
        ...
        if(pixelX<imageWidth && pixelY < imageHeight){
            for(size_t i=0; i<maxNumberCircles; i++){
                if(inSection[i]){
                    int index = indexCicle + i;
